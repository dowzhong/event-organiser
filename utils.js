...
module.exports = {
    ...
    async createEventPost(guild, event) {
        ...
        const eventDate = this.serverToLocalTime(event.date, dbGuild.utc_offset).toLocaleString('en-GB').split(':');
        eventDate.pop();
        const eventDateString = `${days[eventDate.getDay()]}, ${eventDate.join(':')}`;
